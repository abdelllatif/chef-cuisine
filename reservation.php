...
<nav class="bg-gray-900 hidden md:flex text-white p-4 justify-between items-center shadow-md">
    ...
    <div>
        <a href="SignIn.php">
            <button class="bg-orange-500 px-5 py-2 ml-2 rounded-xl hover:bg-black transition-all">Sign In</button>
        </a>
        <a href="SignUp.php">
            <button class="bg-orange-500 px-5 py-2 ml-2 rounded-xl hover:bg-black transition-all">Sign Up</button>
        </a>
    </div>
</nav>
                ...
                <select name="plats[]" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500" required>
                    ...
                    if ($result->num_rows > 0) {
                        while ($plat = $result->fetch_assoc()) {
                            echo "<option value=''"
. $plat['id'] . "'>" . htmlspecialchars($plat['title']) . "</option>";
                        }
                    } else {
...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <table class="w-full table-auto border-collapse border border-gray-300">
        ...
        <tbody id="reservationsTableBody">
            ...
            $sql = "SELECT r.id, m.title, r.date, r.time, r.number_of_people, r.status FROM reservations r JOIN menu m ON r.menu_id = m.id WHERE r.user_id = ?";
            ...
            while ($reservation = $result->fetch_assoc()) {
                echo "<tr>
                    <td class='border border-gray-300 p-2'>" . htmlspecialchars($reservation['title']) . "</td>
                    <td class='border border-gray-300 p-2'>" . htmlspecialchars($reservation['date']) . "</td>
                    <td class='border border-gray-300 p-2'>" . htmlspecialchars($reservation['time']) . "</td>
                    <td class='border border-gray-300 p-2'>" . htmlspecialchars($reservation['number_of_people']) . "</td>
                    <td class='border border-gray-300 p-2'>" . htmlspecialchars($reservation['status']) . "</td>
...
<footer class="bg-gray-800 mt-32 text-white mt-10">
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div>
                <h4 class="text-xl font-bold mb-4">About Us</h ```html
                <ul class="space-y-2">
